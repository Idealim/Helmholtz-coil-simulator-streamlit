...
def helmholtz_coil(r1, r2, d, mf, nf, ms, ns, R=0.04, step=0.1, compensation=False) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    while x <= d: # x 
        ...
        actual_values.append(calculate_actual_value(x, d, compensation))
        
        x += step
        
    return np.array(results_Oe), np.array(actual_values)

def calculate_actual_value(x, d, line_compensation=False) -> float: 
    return -15 * x / d + 26 if line_compensation else -15 * x / d + 25
...
def loss_function(space) -> float:
    actual_values, predicted_values = helmholtz_coil(
        ...
        R=space['R'],
        step=space['step'],
        compensation=space['compensation']
    )
